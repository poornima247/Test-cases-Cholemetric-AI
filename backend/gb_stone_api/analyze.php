...
if (move_uploaded_file($tempPath, $originalPath)) {
    ...
    if ($checkImg) {
        ...
        // 1. Check aspect ratio. CT scans are typically square or landscape, rarely tall portraits like receipts/phone screenshots
        if ($h > $w * 1.2) {
            $isMedicalImage = false;
        } else {
            // 2. Check for color. CT scans are grayscale.
            $colorPixels = 0;
            $totalSampled = 0;
            for ($x = 0; $x < $w; $x += 15) {
                for ($y = 0; $y < $h; $y += 15) {
                    $rgb = imagecolorat($checkImg, $x, $y);
                    $r = ($rgb >> 16) & 0xFF;
                    $g = ($rgb >> 8) & 0xFF;
                    $b = $rgb & 0xFF;
                    $avg = ($r + $g + $b) / 3;
                    $variance = abs($r - $avg) + abs($g - $avg) + abs($b - $avg);
                    if ($variance > 40) $colorPixels++;
                    $totalSampled++;
                }
            }
            if ($totalSampled > 0 && ($colorPixels / $totalSampled) > 0.05) {
                $isMedicalImage = false; // More than 5% colorful pixels
            }
        }
    ...
    $baseUrl = "http://" . $host . "/cholemetric_backend/gb_stone_api/uploads/";
